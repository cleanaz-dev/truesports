      ...
      <div className="mx-auto max-w-7xl px-4 pt-10 sm:px-6 lg:px-8">
        ...
        <header className="mb-12 flex flex-col items-center justify-center gap-4 text-center">
          ...
          <p className="max-w-xl text-muted-foreground sm:text-lg">
            Premium streetwear built for the culture. Rep the brand that runs the game.
          </p>
        </header>
        ...
        <div className="mb-8 flex items-center justify-between border-b border-border pb-4">
          <div className="flex items-center gap-4">
            <span className="font-display text-sm font-bold uppercase tracking-wide">All Gear</span>
            <span className="text-muted-foreground">/</span>
            <span className="font-display text-sm font-bold uppercase tracking-wide text-muted-foreground transition-colors hover:text-foreground cursor-pointer">Tees</span>
            <span className="text-muted-foreground">/</span>
            <span className="font-display text-sm font-bold uppercase tracking-wide text-muted-foreground transition-colors hover:text-foreground cursor-pointer">Hoodies</span>
          </div>
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
          {merchItems.map((item) => (
            <div key={item.id} className="group relative flex flex-col gap-4">
              ...
              <div className="relative aspect-[4/5] w-full overflow-hidden rounded-2xl bg-muted/50 p-6 transition-colors hover:bg-muted">
                
                {/* Badge (if exists) */}
                {item.badge && (
                  <div className="absolute left-4 top-4 z-10 rounded bg-foreground px-2.5 py-1 font-display text-[10px] font-bold uppercase tracking-widest text-background">
                    {item.badge}
                  </div>
                )}
                
                {/* Product Image (fallback to placeholder if no real image) */}
                <img 
                  src={item.image} 
                  alt={item.name} 
                  onError={(e) => { e.currentTarget.src = "/placeholder.svg" }} // Fallback
                  className="size-full object-contain transition-transform duration-700 ease-out group-hover:scale-110 drop-shadow-xl"
                />

                {/* Quick Add Overlay */}
                <div className="absolute bottom-4 left-4 right-4 translate-y-4 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                  <button className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary/90 py-3 font-display text-sm font-bold uppercase tracking-wider text-primary-foreground backdrop-blur-sm transition-colors hover:bg-primary">
              ...
              <div className="flex flex-col gap-1 px-1">
                ...
                <span className="text-sm text-muted-foreground">Unisex</span>
              </div>
            </div>
          ))}
        </div>
