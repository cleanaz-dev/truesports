...
import { signIn } from "@/lib/auth-client"; // Your exported Better Auth client
...
export function SignInPage() {
  ...
  const handleEmailSignIn = async (e: React.FormEvent) => {
    ...
    // Better Auth handles the API call automatically
    ...
    <div className="flex min-h-screen items-center justify-center bg-muted/30 p-4">
      <Card className="w-full max-w-md shadow-lg">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Sign in to your TrueSports account
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleEmailSignIn} className="space-y-4">
            ...
            <Button type="submit" className="w-full" disabled={loading !== false}>
              {loading === "email" ? (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ) : null}
              Sign In
