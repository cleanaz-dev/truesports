...
export default async function LeaguePage({ params }: Params) {
    ...
    if (!currentLeague) {
        return (
            <div className="flex min-h-[50vh] items-center justify-center">
                <h1 className="text-2xl font-bold text-muted-foreground">League not found.</h1>
            </div>
        )
    }
    ...
    if (otherArticles.length < 4) {
        const filler = articles.filter(a => a.id !== featuredArticle.id && !otherArticles.find(oa => oa.id === a.id));
    ...
    return (
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <div className="mb-8 flex items-center justify-between border-b border-border pb-4">
                <h1 className={`font-display text-4xl font-black uppercase tracking-tight ${leagueAccent(currentLeague)}`}>
                    {currentLeague} News
                </h1>
            </div>
                ...
                <aside className="lg:col-span-4 flex flex-col gap-8">
                    <div className="rounded-2xl border border-border bg-card/40 p-6 shadow-sm">
                        <div className="mb-6 flex items-center justify-between border-b border-border pb-3">
                            ...
                            <a href="#" className="font-display text-xs font-semibold uppercase tracking-wide text-muted-foreground hover:text-foreground">
                                @thepressbox
                            </a>
                        </div>
                        <div className="flex flex-col gap-6">
