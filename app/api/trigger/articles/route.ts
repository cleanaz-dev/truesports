...
const VALID_LEAGUES = Object.values(League) as string[];
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      "Expected AI_NEWS_API_KEY set:",
      apiKey ? `yes (len ${apiKey.length})` : "MISSING/undefined",
    );
    ...
    const rawLeague = (searchParams.get("league") || "NBA").toUpperCase();

    if (!VALID_LEAGUES.includes(rawLeague)) {
      console.log(`Unsupported league param: ${rawLeague}`);
      return NextResponse.json(
        { error: `Unsupported league: ${rawLeague}` },
        { status: 400 },
      );
    }
    ...
    if (newArticles.length === 0) {
      return NextResponse.json({
        message: `No new articles for ${league}. AI budget saved!`,
      });
    }
    ...
    const command = createCommand({
      functionName:
        process.env.AWS_LAMBDA_NEWS_WORKER_NAME || "your-lambda-function-name",
      payload: payload,
